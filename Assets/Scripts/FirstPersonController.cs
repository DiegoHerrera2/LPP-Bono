...
namespace StarterAssets
{
	...
	public class FirstPersonController : MonoBehaviour
	{
		...
		private void Update()
		{
			JumpAndGravity();
			GroundedCheck();
			if (!_health.dead) Move();
            
        }

		private void LateUpdate()
		{
            if (!_health.dead) CameraRotation();
        }
		...
		private void JumpAndGravity()
		{
			if (Grounded)
			{
				...
				// Fall damage
				if(_fallTimeoutDelta >= 1)
				{
					float amount = Mathf.Clamp(_fallTimeoutDelta, 0, 3f) / 3f;
					_health.Damage((int)(amount * 30));
					_audioSource.Play();
